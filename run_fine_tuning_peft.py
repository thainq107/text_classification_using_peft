...
class ModelArguments:
    ...
    model_name_or_path: str = field(
        default="google/flan-t5-xl", metadata={"help": "Path to pretrained model or model identifier from huggingface.co/models"}
    )
...
class PeftArguments:
    """
    Arguments pertaining to which PEFT we are going to fine-tune from.
    """
    ...
    use_prefix: Optional[bool] = field(
        default=False, metadata={"help": "Use Prefix-Tuning or not"}
    )
    prefix_num_virtual_tokens: Optional[float] = field(
        default=20, metadata={"help": "Number of tokens is unfreeze."}
    )

    # Prompting-Tuning Config
    use_prompt: Optional[bool] = field(
        default=False, metadata={"help": "Use Prompt-Tuning or not"}
    )
    prompt_num_virtual_tokens: Optional[float] = field(
        default=20, metadata={"help": "Number of tokens is unfreeze."}
    )
    
def main():
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, Seq2SeqTrainingArgumentsCustom, PeftArguments))
    ...
    elif peft_args.use_prefix:
        training_args.hub_model_id = training_args.hub_model_id + "-prefix"
        save_folder_path = save_folder_path + "-prefix"
    elif peft_args.use_prompt:
        training_args.hub_model_id = training_args.hub_model_id + "-prompt"
        save_folder_path = save_folder_path + "-prompt"
    ...
    train(model_args, data_args, training_args, peft_args)
